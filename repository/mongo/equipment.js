...
class EquipmentMongoRepository extends AppMongoRepository {
	async search(correlationId, gameSystemId, search) {
		...
		response.results = await this._fetchExtract(await this._find(collection, { $and: [ { 'name': { $regex: new RegExp(search.name, 'i') } }, { 'gameSystemId' : gameSystemId } ] }), this._initResponseExtract(correlationId));
		return response;
	}
}
